...
public class ClientHandler {
    ...
    /**
     * Same behavior as {@link SheepWoolLayer}: vanilla wool when {@code moreMobVariantsVariant == 0} or the sheep is a baby,
     * mod wool texture and full white tint when {@code moreMobVariantsVariant > 0} on adults (matches prior {@code 0xFFFFFFFF}).
     */
        ...
        public void submit(PoseStack poseStack, SubmitNodeCollector collector, int packedLight, SheepRenderState state, float limbSwing, float limbSwingAmount) {
            if (!state.isSheared) {
                ...
                if (state.isInvisible) {
                    ...
                    if (state instanceof MmvSheepRenderState m && m.moreMobVariantsVariant > 0 && !state.isBaby) {
